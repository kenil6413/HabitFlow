...
| App accomplishes approved requirements                                        | READY  | Features implemented across `backend/routes/*`, `frontend/js/*`; final instructor validation needed |
| App is usable and includes instructions                                       | PASS   | In-app guidance on Home (`frontend/home.html`), plus README usage steps                             |
...
| Code properly organized by pages/modules                                      | PASS   | Separate HTML/CSS/JS modules in `frontend/`; backend split in `backend/`                            |
| JS organized in modules                                                       | PASS   | ESM modules across frontend/backend                                                                 |
| Client-side rendering using vanilla JS                                        | PASS   | `frontend/js/*` uses DOM rendering with no framework                                                |
...
| Uses at least 2 Mongo collections with CRUD support                           | PASS   | `users`, `habits`, `journal` via `backend/routes/*.js`                                              |
| Database has >1000 records                                                    | READY  | `scripts/seed-rubric-data.js`; run `npm run seed:rubric`                                            |
| Uses Node + Express                                                           | PASS   | `backend/server.js`                                                                                 |
...
| CSS organized by modules                                                      | PASS   | `frontend/css/auth.css`, `home.css`, `dashboard.css`, `friends.css`, `common.css`                   |
...
| Backend does not use CommonJS (`require`)                                     | PASS   | ESM imports in `backend/`                                                                           |
